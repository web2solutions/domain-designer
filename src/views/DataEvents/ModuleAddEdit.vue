...
import { useDataEventsStore, useDomainsStore, useAlertStore } from '@/stores';
...
const entityStore = useDataEventsStore();
...
async function save() {
  const data = getHash();
  const record = await entityStore.create(data as IDataEventCreateDTO);
...
async function update() {
  ...
  const data = getHash();
  console.log(data)
  const record = await entityStore.update(id as string, data as IDataEventCreateDTO);
